...
from app.services.publish_instagram import instagram_service
...
async def generate_daily_post():
    ...
    try:
        ...
        success, post, error = await state_engine.generate_post(
            ...
            publish_to_instagram=instagram_service.is_enabled()
        )
        
        if success:
            logger.info(f"✅ Post generado exitosamente: ID {post.id}")
            if post.published:
                logger.info(f"📱 Publicado en Instagram: {post.instagram_media_id}")
        else:
